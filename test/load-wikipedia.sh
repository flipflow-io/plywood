...
# Load the Wikipedia test dataset into a disposable Druid for functional tests.
# DRUID_HOST is required and has no default: the ingestion REPLACEs the "wikipedia"
# datasource, so it must never reach a production cluster (on flipflow-dev,
# localhost:8182 is the SSH tunnel to the production router).
# Dataset: 39244 rows of Wikipedia edits from 2015-09-12.

: "${DRUID_HOST:?DRUID_HOST is not set: point it at a disposable test Druid router, never at production}"
